...
export const imageUsage = {
  request: (id) => action(types.IMAGE_USAGE.REQUEST, { id }),
  success: (id, response) => action(types.IMAGE_USAGE.SUCCESS, { id, response }),
  failure: (id, error) => action(types.IMAGE_USAGE.FAILURE, { id, error }),
};
